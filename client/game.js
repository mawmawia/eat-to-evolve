...
function createAllSprites() {
  for (let id in players) {
    const p = players[id];
    const sprite = this.add.sprite(p.x, p.y, p.type).setScale(p.size);
    sprite.id = id; playerSprites.set(id, sprite);
  }
  for (let id in foods) {
    const f = foods[id];
    const sprite = this.add.image(f.x, f.y, f.type).setScale(0.5);
    sprite.id = id; foodSprites.set(id, sprite);
  }
}
